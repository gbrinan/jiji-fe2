...
import type { HrtRelativeQuestionnaire, HrtRelativeResult, HrtAnswerItem } from "@/lib/types";
...
export default function HrtRelativePage() {
  ...
  const [questionnaire, setQuestionnaire] = useState<HrtRelativeQuestionnaire | null>(null);
  ...
  useEffect(() => {
    api.get<HrtRelativeQuestionnaire>("/api/v1/survey/hrt/relative")
  ...
  if (loading || !questionnaire) {
    return (
      <div>
        <Header title="상대적 금기사항 확인" showBackButton />
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const result = await api.post<HrtRelativeResult>("/api/v1/survey/hrt/relative", { answers: payload });
      sessionStorage.setItem("hrt_result", JSON.stringify(result));
    ...
    <div className="min-h-dvh flex flex-col">
      ...
      <div className="px-5 pt-4 flex-1 flex flex-col">
        ...
        <Card variant="elevated" padding="lg" className="mt-4 flex-1">
          {q.category && <p className="text-xs font-medium text-primary-500 mb-1">{q.category}</p>}
